...
public class WaypointArgumentMapper extends AbstractArgumentMapper<CommandSender, Waypoint> {
    ...
    public WaypointArgumentMapper(
            final BiFunction<WaypointService, Player, Set<Waypoint>> valueProvider,
    ...
    public Waypoint tryMap(final CommandContext<CommandSender> context, final CommandInputTokenizer input) throws ArgumentMappingException, MissingInputException {
        ...
        if (!(context.source() instanceof Player player)) {
            throw new IllegalStateException("Command source was not a player. Perhaps command chain '%s' has incorrect conditions?".formatted(context.chain()));
        }
        ...
        return candidate.orElseThrow(() -> new VerboseArgumentMappingException(config -> Messages.WAYPOINT__NO_SUCH_WAYPOINT.from(config, value).comp()));
    }
